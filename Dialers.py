...
# Define the years and months for the filter
YEARS = [2024, 2025, 2026] 
# Updated MONTH_NAMES to include all months
MONTH_NAMES = [
...
def get_attended_dialers(df_attendance, selected_year, selected_month_index):
    ...
    if DIALER_COLUMN not in df_attendance_copy.columns or 'date' not in df_attendance_copy.columns:
        # Fallback list if required columns are missing entirely
        return ["All Dialers", "SA2", "SA3", "SA4", "HU1"]
    ...
    # FORCE INJECT REQUESTED DIALERS so they are always available in the filter
    required_dialers = ["SA2", "SA3", "SA4", "HU1"]
    for d in required_dialers:
        if d not in dialers:
            dialers.append(d)
            
    dialers = sorted(dialers)
...
def show_sales_dashboard(df_attendance, df_sales, df_oplans):
    ...
    # 4a. Year Selector
    selected_year = st.sidebar.selectbox("Select Year", options=YEARS, index=default_year_index, key="year_sales")

    # 4b. Month Selector (multi-select)
    # Automatically gets the current month name (e.g., "March")
    default_month_name = datetime.now().strftime("%B")
    
    # Fallback in case the system locale produces a month name not in our list
    if default_month_name not in MONTH_NAMES:
        default_month_name = "March"
...
def show_oplans_dashboard(df_attendance, df_oplans):
    ...
    # Year selector
    selected_year_op = st.sidebar.selectbox("Select Year (Oplans)", options=YEARS, index=default_year_index, key="year_oplans")
...
def show_others_page(df_others, df_oplans, df_attendance, df_sheet2):
    ...
    # Year selector
    selected_year_oth = st.sidebar.selectbox("Select Year (Others)", options=YEARS, index=default_year_index, key="year_others")
